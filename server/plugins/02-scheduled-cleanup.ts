...
export default defineNitroPlugin(async () => {
  ...
  if (cleanupOnStartup) {
    ...
    setTimeout(() => {
      ...
      cleanupExpiredData()
        .then((result) => console.log('[Cleanup] Initial cleanup completed:', result))
        .catch((error) => console.error('[Cleanup] Initial cleanup failed:', error))
    }, 1000) // 1 second delay
  }
  ...
  const timer = setInterval(() => {
    ...
    cleanupExpiredData()
      .then((result) => console.log('[Cleanup] Scheduled cleanup completed:', result))
      .catch((error) => console.error('[Cleanup] Scheduled cleanup failed:', error))
  }, intervalMs)
